local highlights = require "asciiville.highlights"
...
local prepare_environment = function()
    ...
    if vim.g.asciiville_style == "lighter" then
    ...
    if not settings.disable.colored_cursor then
        ...
        local exit_group  = vim.api.nvim_create_augroup("AsciivilleExit", { clear = true })
...
local apply_contrast = function(contrast_settings)
    local group = vim.api.nvim_create_augroup("Asciiville", { clear = true })

    -- clean up autogroups if the theme is not asciiville
    vim.api.nvim_create_autocmd("ColorScheme", { callback = function()
        if vim.g.colors_name ~= "asciiville" then
            vim.api.nvim_del_augroup_by_name("Asciiville")
        end
    end, group = group })
...
M.load = function()
    ...
    if settings.italics then
        vim.notify ("Asciiville.nvim had breaking changes", vim.log.levels.WARN)
